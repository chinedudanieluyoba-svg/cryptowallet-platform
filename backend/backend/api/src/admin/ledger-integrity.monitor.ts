...
export class LedgerIntegrityMonitor implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    const intervalMs = Number(process.env.LEDGER_INTEGRITY_INTERVAL_MS ?? '900000')
    ...
    this.timer = setInterval(() => {
      ...
      this.runAudit()
        .catch((error) => {
          ...
          this.alertsService.alertReconciliationCrash('ledger_integrity', error)
        })
        .finally(() => {
          this.running = false
        })
  ...
  private async runAudit() {
    ...
    const flaggedCount = await this.adminService.flagInconsistentWallets(result.results)
