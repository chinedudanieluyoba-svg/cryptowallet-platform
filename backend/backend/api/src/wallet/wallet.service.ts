...
export class WalletService {
  ...
  async creditWallet(input: CreditWalletInput): Promise<WalletResponse> {
    const requestId = this.requestIdStorage.getRequestId()
     ...
     // Check idempotency
     if (idempotencyKey) {
       const idempotencyResult = await this.idempotencyService.checkIdempotency(idempotencyKey)
    ...
    if (input.source === 'admin' && input.actorRole !== 'ADMIN') {
      ...
      throw new ForbiddenException('Credits are allowed only from admin actions')
    }
    ...
    if (input.source === 'webhook') {
      if (!input.verifiedWebhook || !input.providerEventId) {
        ...
        throw new ForbiddenException('Credits are allowed only from verified webhook events')
      }
    }
    ...
    return await this.prisma.$transaction(async (tx) => {
       ...
       const wallet = await this.concurrencyLockService.lockWallet(tx, input.walletId)
  ...
  private async assertLedgerMatchesBalance(walletId: string, walletBalance?: number) {
    const requestId = this.requestIdStorage.getRequestId()
    ...
    if (!this.balancesMatch(balanceToCheck, ledgerBalance)) {
      ...
      throw new InternalServerErrorException('Wallet ledger integrity check failed')
    }
  }
