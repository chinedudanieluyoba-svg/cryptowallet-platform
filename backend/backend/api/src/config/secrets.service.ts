...
export class SecretsService {
  ...
  validateSecrets(): SecretsReport {
    ...
    if (nodeEnv === 'production') {
      secrets.push(this.checkSecret('DATABASE_URL_PROD', 'DB_CREDENTIALS_ROTATION_DATE'))
    }

    // Payment provider secrets
    secrets.push(
      this.checkSecret('MOONPAY_WEBHOOK_SECRET', 'MOONPAY_SECRET_ROTATION_DATE')
    )
  ...
  private validateNoHardcodedSecrets(warnings: string[]): void {
    ...
    for (const key of suspiciousKeys) {
      ...
      if (value.includes('your_') || value.includes('example_') || value.includes('test_')) {
        warnings.push(
      ...
      if (value.length < 16 && key.includes('SECRET')) {
        warnings.push(`⚠️  ${key} is too short - use at least 16 characters (recommend 32+)`)
      }
    }
  }
  ...
  validateDatabasePrivileges(): { ok: boolean; issues: string[] } {
    ...
    if (dbUrl) {
      try {
        ...
        if (username === 'root' || username === 'postgres' || username === 'admin') {
          issues.push(
