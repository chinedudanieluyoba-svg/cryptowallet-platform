...
export class ProductionConfigService {
  ...
  private static getCorsOrigins(nodeEnv: string): string[] {
    if (nodeEnv === 'production') {
      ...
      if (!allowedOrigins) {
        console.warn('⚠️  CORS_ALLOWED_ORIGINS not set in production - defaulting to none')
        return []
      }
    ...
    if (nodeEnv === 'staging') {
      ...
      const allowedOrigins = process.env.CORS_ALLOWED_ORIGINS
      if (!allowedOrigins) {
        console.warn('⚠️  CORS_ALLOWED_ORIGINS not set in staging - allowing localhost')
  ...
  private static getLogLevel(nodeEnv: string): 'error' | 'warn' | 'log' | 'debug' | 'verbose' {
    if (nodeEnv === 'production') {
  ...
  static logConfig(): void {
    ...
    console.log(`   Stack Traces: ${config.exposeStackTraces ? 'EXPOSED (dev only)' : 'HIDDEN (secure)'}`)
    console.log(`   CORS Origins: ${config.corsOrigins.length > 0 ? config.corsOrigins.join(', ') : 'NONE (locked)'}`)
    console.log('')
  }
}
